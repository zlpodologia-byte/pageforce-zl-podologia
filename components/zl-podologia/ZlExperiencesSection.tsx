    ...
    >
      <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
        ...
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-[0.72rem] font-semibold uppercase tracking-[0.24em] text-[#0F6B46]">
            {eyebrow}
          </p>
          <h2
            id="experiencias-presentes-title"
            className="mt-3 font-display text-[clamp(2rem,3.6vw,2.8rem)] leading-[1.05] tracking-[-0.02em] text-[#26302B]"
          >
            {title}
          </h2>
          <p className="mt-4 text-[1rem] leading-[1.55] text-[#6F746F]">
            {subtitle}
          </p>
        </div>
            ...
            <ZlSceneEnvelope key={group.id} tone="white">
              <div className="p-6 lg:p-8">
                ...
                <div className="mb-6 max-w-3xl">
                  <p className="text-[0.66rem] font-semibold uppercase tracking-[0.22em] text-[#0F6B46]">
                    {group.eyebrow}
                  </p>
                  <h3 className="mt-2 font-display text-[clamp(1.4rem,2.2vw,1.85rem)] leading-[1.15] text-[#26302B]">
                    {group.title}
                  </h3>
                  <p className="mt-2 text-[0.95rem] leading-[1.5] text-[#6F746F]">
                    {group.intro}
                  </p>
                </div>
        ...
        {/* Closing */}
        <p className="mx-auto mt-8 max-w-2xl text-center text-[0.9rem] leading-[1.6] text-[#6F746F]">
          {closing}
        </p>
      </div>
    </section>
  ...
  return (
    <article
      className={`relative flex flex-col overflow-hidden rounded-[24px] border bg-white transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-[2px] ${
        card.featured
          ?"border-[#0F6B46]/40 shadow-[0_18px_50px_rgba(15,107,70,0.18)]"
          : "border-[#E6D2C7] shadow-[0_12px_35px_rgba(154,127,92,0.08)]"
      }`}
    >
      {/* Featured badge */}
      {card.featured && card.featuredLabel ?(
        <div className="absolute right-3 top-3 z-10 rounded-full bg-[#0F6B46] px-3 py-1 text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-white">
          {card.featuredLabel}
        </div>
      ) : null}
      ...
      {/* Imagem do card */}
      {card.image ?(
        <div className="relative h-48 w-full overflow-hidden bg-[#F8F7F4] md:h-52">
          <Image
            src={card.image.src}
            alt={card.image.alt}
            fill
      ...
      <div className="flex flex-1 flex-col p-5">
        <p className="text-[0.62rem] font-semibold uppercase tracking-[0.22em] text-[#0F6B46]">
          {card.eyebrow}
        </p>
        <h4 className="mt-2 font-display text-[1.3rem] leading-[1.15] text-[#26302B]">
          {card.title}
        </h4>
        <p className="mt-1 text-[0.82rem] font-medium uppercase tracking-[0.12em] text-[#6F746F]">
          {card.duration}
        </p>

        {/* Includes checklist */}
        <ul className="mt-4 space-y-2 text-[0.88rem] leading-[1.45] text-[#6F746F]">
          {card.includes.map((item) => (
            <li key={item} className="flex items-start gap-2">
              <span
                aria-hidden="true"
                className="mt-[6px] inline-block h-1.5 w-1.5 rounded-full bg-[#8EA08E]"
              />
              <span>{item}</span>
            </li>
          ))}
        ...
        <div className="mt-5 flex flex-col gap-3 border-t border-[#E6D2C7] pt-4">
          <div className="flex items-baseline justify-between">
            <span className="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-[#6F746F]">
              Investimento
            </span>
            <span className="font-display text-[1.4rem] leading-none text-[#26302B]">
              {card.price}
            </span>
          </div>
          <a
            ...
            className={`group inline-flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-[0.85rem] font-semibold transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] active:scale-[0.98] ${
              card.featured
                ?"bg-[#0F6B46] text-white hover:bg-[#2f7a60]"
                : "border border-[#26302B] bg-white text-[#26302B] hover:bg-[#26302B] hover:text-white"
            }`}
